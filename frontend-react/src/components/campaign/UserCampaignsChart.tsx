...
export function UserCampaignsChart() {
  ...
  const chartData: { month: number; campaigns: number }[] = [];
  userCampaigns.map((campaign: Campaign) => {
    ...
    const idx = chartData.findIndex(
      (item: { month: number; campaigns: number }) => item.month === month,
    );
  ...
  chartData.sort(
    (a: { month: number; campaigns: number }, b: { month: number; campaigns: number }) =>
      a.month - b.month,
  );
